...
export default function AlbumsList(props: FileData) {
  const { inputFile } = props;

  const [albums, setAlbums] = useState<string[]>([]);
  const [albumsNumbers, setAlbumsNumbers] = useState<number>(50);
  ...
  useEffect(() => {
    ...
    inputFile.forEach((e) => {
      albumsNames.push(e.master_metadata_album_album_name || "Unknown Album");
    });
    setAlbums(albumsNames);
  }, [inputFile]);

  const ranking = count(albums, "albumName", "playCount");
